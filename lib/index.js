...
import { distinctUntilChanged, map, shareReplay, tap, scan, startWith, skip, buffer, filter, } from "rxjs/operators";
...
const pageFound$ = pageFoundSubject$.pipe(buffer(navigationEvents$.pipe(filter((env) => env === "navEnd"))), map((buf) => buf[buf.length - 1]));
...
export class EagRouter extends RouterMix(LitElement) {
    ...
    connectedCallback() {
        ...
        queryString$.subscribe(item => {
            console.log(item);
        });
        this.addToSub(pageFound$.pipe(tap((found) => {
            if (found === false) {
    ...
    async changeRoute(context) {
        try {
            ...
            this.observerHandler(pageFoundSubject$, myWindow, pendingSubject$, context.querystring, queryStringSubject$, 'parent');
            oldPath = elem.path;
        }
        catch (error) {
            console.error(error);
...
export class EagRouterChild extends RouterMix(LitElement) {
    ...
    childRouterCheck() {
        return (this.element.querySelector("eag-router-child") ||
    ...
    async renderView(path) {
        ...
        try {
            ...
            const oldElem = this.element;
            this.element = stringToHTML(elem.component || "<eag-router-empty></eag-router-empty>");
            this.requestUpdate("element", oldElem);
            // Using intersection observer to check when element is loaded
            this.observerHandler(pageFoundSubject$, myWindow, pendingSubject$, "", queryStringSubject$);
        }
        catch (error) {
            console.error(error);
            pendingSubject$.next(-1);
